...
#include "Account.h"
#include "Customer.h"
#include <algorithm> //to use find to search vector 
	...
	*/
	Account * add_account(Customer *cust, std::string account_type)
	{
		...
		if (account_type == "savings") {
			...
			std::cout << "Got lost in add_account (cust, account_type(). Could not create account\n";
		// Factory method for creating a Account object (could be a Saving_Account or a Checking_Account).
		accounts.push_back(acct);//pushes account into vector for specific customer
		return acct;
	}
...
public:
	...
	*/
	...
	{
		Customer *cust;// Depending on the customer type, we want to create an Adult, Senior, or Student object.
		if (cust_type == "adult") {
			...
			std::cout << "Could not creat an account. Exiting program. " << std::endl;
			return 0;
		}
