...
int n = 0; //to iterate through the recursion
										  /**
...
bool PlaceQueen(int column) {
	...
	if (column == NUMBER_COLS) { // Base Condition
			...
			if (PlaceQueen(column - 1)) { //recursion iside of the recursion called in the else statement below. 
										//If below is true and it finds a dead, it backtracks to find the correct solution. There are 92.
				return true;
			} // Successfully assigned all column queens in the board.  We can just return true.
	}
	else {
		...
		for (int row = 0; row < RowPlacement.size(); ++row) {
			...
			n++; //every time it gets unassigned 
...
int main()
{
	...
	cout << endl << "Number of backtracks the iterator counts from the recusrion (i.e. # of tries): " << n << endl;
}