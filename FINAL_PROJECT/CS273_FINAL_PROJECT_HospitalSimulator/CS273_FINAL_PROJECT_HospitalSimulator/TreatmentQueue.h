...
public:
	...
	void updateDoctor(int clock) {
		if (!doctorQueue.empty()) { //of there is a doctor in the queue..
			...
			if ((clock - patient->setStartTreatmentTime) > patient->totalTreatmentTime) { //if there is extra time, pop a doctor from the queue
				doctorQueue.pop();
		...
		if (doctorQueue.empty() || waitingRoomQueue->numberOfDoctors > 0) {
			while (waitingRoomQueue->numberOfDoctors > 0 && (!waitingRoomQueue->doctorQueue.empty())) {
	...
	void updateNurse(int clock) {
		...
		if (nurseQueue.empty() || waitingRoomQueue->numberOfNurses > 0) {
			while (waitingRoomQueue->numberOfNurses > 0 && (!waitingRoomQueue->nurseQueue.empty())) {
